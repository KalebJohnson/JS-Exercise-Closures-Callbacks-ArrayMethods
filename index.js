...
function firstNamesAllCaps(runners) {
  ...
  runners.map((runner, index) => {
      Caps[index] = runner.first_name.toUpperCase();
  })
  return Caps
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let smallShirts=[]
smallShirts = runners.filter(runner => runner.shirt_size === 'S')
return smallShirts
}
...
function tallyUpDonations(runners) {
  let totalIncome = []
totalIncome = runners.reduce((totalAmount, runner) => {
...
function counterMakerWithLimit(limit) {
  ...
  let counter = counterMakerWithLimit(4)
